...
#      lupdate6 ../*.ui -ts uifiles.ts -source-language en_US -target-language $lang -no-obsolete
      ...
      pylupdate6 ../../../*.py ../../../SurfaceTests/*.py -ts pyfiles.ts --verbose
      # Join uifiles.ts and pyfiles.ts files to Surface.ts
      lconvert -i uifiles.ts pyfiles.ts -o Surface.ts --verbose
      # Join Surface.ts to exist Surface_(language).ts file ( -no-obsolete)
      lconvert -i Surface.ts Surface_zh-TW.po -o Surface_$lang.ts -target-language zh_TW -sort-contexts --verbose
#      lconvert -i Surface.ts Surface_$lang.ts -o Surface_$lang.po -of po -target-language $lang -sort-contexts
      lconvert -i Surface.ts -o Surface.pot -of pot -target-language en-US
      lconvert -i title.ts Surface_zh-TW.ts -o Surface_zh-TW.po -of po -target-language zh_TW --verbose
      # (Release) Creation of *.qm file from Surface_(language).ts
      lrelease Surface_$lang.ts
      # Delete unused files
#      rm uifiles.ts
      cd ..
      dir2qrc Surface
      ...
      # rm Surface.ts
#      lconvert -i Surface_$lang.po -if po -o Surface.pot -of pot -source-language en_US -sort-contexts
   else
      ...
      # Join uifiles.ts and pyfiles.ts files to Surface_$lang.ts
      lconvert -i uifiles.ts pyfiles.ts -o Surface_$lang.ts
      # Delete unused files
#      rm uifiles.ts
      rm pyfiles.ts
   fi
