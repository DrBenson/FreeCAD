...
#      lupdate6 ../preferences/*.ui -ts uifiles.ts -source-language en_US -target-language $lang -no-obsolete
      ...
      pylupdate6 ../../../*.py -ts pyfiles.ts --verbose
      # Join uifiles.ts and pyfiles.ts files to Tux.ts
      lconvert -i uifiles.ts pyfiles.ts -o Tux.ts --verbose
      # Join Tux.ts to exist Tux_(language).ts file ( -no-obsolete)
      lconvert -i title.ts Tux.ts Tux_zh-TW.po -o Tux_$lang.ts -target-language $lang -sort-contexts --verbose
#      lconvert -i Tux.ts Tux_$lang.ts -o Tux_$lang.po -of po -target-language $lang -sort-contexts
      lconvert -i Tux.ts -o Tux.pot -of pot -target-language en-US
      lconvert -i title.ts Tux.ts Tux_zh-TW.po -o Tux_zh_TW.po -of po -target-language zh_TW --verbose
      # (Release) Creation of *.qm file from Tux_(language).ts
      lrelease Tux_$lang.ts
      # Delete unused files
#      rm uifiles.ts
      rm pyfiles.ts uifiles.ts
      # rm Tux.ts
#      lconvert -i Tux_$lang.po -if po -o Tux.pot -of pot -source-language en_US -sort-contexts
   else
      ...
      # Join uifiles.ts and pyfiles.ts files to Tux_$lang.ts
      lconvert -i uifiles.ts pyfiles.ts -o Tux_$lang.ts
      # Delete unused files
#      rm uifiles.ts
      rm pyfiles.ts
   fi
